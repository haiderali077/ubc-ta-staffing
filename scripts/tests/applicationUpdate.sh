#!/bin/bash
# Test script for applicationUpdate.test.ts

echo "🧪 Running Application Update Tests..."
...
# Run application update tests
DENO_ENV=test DB_HOST=localhost DB_PORT=5432 DB_NAME=test_db DB_USER=test_user DB_PASSWORD=test_password \
deno test --allow-all --no-check app/src/backend/_tests_/application_updates.test.ts
