...
import {
  getInstructorNotificationPreferences,
  updateInstructorNotificationPreferences,
  type InstructorNotificationPreferences,
} from "../../api/instructorNotificationApi";

const InstructorNotificationPreferencesComponent: React.FC = () => {
  const [prefs, setPrefs] = useState<InstructorNotificationPreferences>({});
  ...
  const handleChange = (
    key: keyof InstructorNotificationPreferences,
  ...
  const handleSave = async () => {
    ...
    setSuccess(false);
    try {
      await updateInstructorNotificationPreferences(prefs);
      setSuccess(true);
      ...
      <div className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <label className="flex items-center gap-3 p-3 rounded-lg border border-gray-100 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors cursor-pointer">
            <input
              type="checkbox"
              checked={!!prefs.ta_request_updates}
              onChange={(e) =>
                handleChange("ta_request_updates", e.target.checked)
              }
              className="w-4 h-4 text-blue-600 bg-gray-100 dark:bg-gray-600 border-gray-300 dark:border-gray-500 rounded focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 checked:bg-blue-600 dark:checked:bg-blue-600"
            />
            <div>
              ...
              <div className="text-sm text-gray-500 dark:text-gray-400">
                Updates about TA requests for your courses
              </div>
            </div>
          </label>
        </div>
...
export default InstructorNotificationPreferencesComponent;
